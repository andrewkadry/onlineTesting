...
router.get('/', function(req, res, next) {
  ...
  db.query(sql, function (err, result) {
    ...
    res.render('exams', { title: 'Exams', exams: result });
  });
});
...
router.post('/add', function(req, res, next) {
  ...
  db.query(sql1, function (err, result) {
    if (err)
      return res.status(500).send(err);
    if(result.length > 0){
      return res.render('addExam', { title: 'Exams', name: name, error: "Name already exist" });
    }
...
router.get('/edit/:id', function(req, res, next) {
  ...
  db.query(sql1, function (err, result) {
    if (err)
      return res.status(500).send(err);
    res.render('editExam', { title: 'Exams', id: result[0].ID, name: result[0].Name, error: "" });
  });
});
...
router.post('/edit', function(req, res, next) {
  ...
  db.query(sql1, function (err, result) {
    ...
    if(result.length > 0){
      if(result[0].ID != id){
        return res.render('editExam', { title: 'Exams', id: id, name: name, error: "Name already exist" });
      }
    }
...
router.get('/topics/:id', function(req, res, next) {
  ...
  db.query(sql1, function (err, result1) {
    ...
    db.query(sql2, function (err, result2) {
      if (err)
        return res.status(500).send(err);
      res.render('addExamTopic', { title: 'Exams', examId: examId, topics: result1, examTopics: result2});
      });
  });
});
