...
router.get('/', function(req, res, next) {
  ...
  db.query(sql, function (err, result) {
    ...
    res.render('questions', { title: 'Questions', questions: result });
  });
});
...
router.get('/add', function(req, res, next) {
  ...
  db.query(sql, function (err, result) {
    if (err)
      return res.status(500).send(err);
    res.render('addQuestion', { title: 'Questions', topics: result });
  });
});
  ...
  db.query(sql1, function (err, result1) {
    if (err)
      ...
      db.query(sql2, function (err, result2) {
        if (err)
          return res.status(500).send(err);
        res.render('editQuestion', { title: 'Questions', id: result1[0][0].ID, name: result1[0][0].Name, topicId: result1[0][0].TopicID, answers: result2, topics: result1[1] });
      });
  });
});
