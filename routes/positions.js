...
router.get('/', function(req, res, next) {
  ...
  db.query(sql, function (err, result) {
    ...
    res.render('positions', { title: 'Positions', positions: result });
  });
});

/* GET add position page */
router.get('/add', function(req, res, next) {
  res.render('addPosition', { title: 'Positions', name: "", error: "" });
});
...
router.post('/add', function(req, res, next) {
  ...
  db.query(sql1, function (err, result) {
    if (err)
      return res.status(500).send(err);
    if(result.length > 0){
      return res.render('addPosition', { title: 'Positions', name: name, error: "Name already exist" });
    }
...
router.get('/edit/:id', function(req, res, next) {
  ...
  db.query(sql1, function (err, result) {
    if (err)
      return res.status(500).send(err);
    res.render('editPosition', { title: 'Positions', id: result[0].ID, name: result[0].Name, available: result[0].Available, error: "" });
  });
});
...
router.post('/edit', function(req, res, next) {
  ...
  db.query(sql1, function (err, result) {
    ...
    if(result.length > 0){
      if(result[0].ID != id){
        return res.render('editPosition', { title: 'Positions', id: id, name: name, available: available, error: "Name already exist" });
      }
    }
